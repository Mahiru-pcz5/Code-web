...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($user_name) && !empty($email) && !empty($pass) && !empty($address) && !empty($phone)) {
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, email, password, address, phone, role, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssss", $user_name, $email, $hashed_password, $address, $phone, $role, $created_at);
